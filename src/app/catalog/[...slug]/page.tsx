...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  return { title: { absolute: `${product.seoTitle ?? product.title}${product.article ? ` — ${product.article}` : ''} — ЩУПЫ.РУ` }, description, alternates: { canonical: `/catalog/${product.slug}` }, openGraph: { title: product.title, description, images: product.images } };
}
...
export default async function CatalogEntityPage({ params, searchParams }: Props) {
  ...
  const productLd = { '@context': 'https://schema.org', '@type': 'Product', name: product.title, ...(product.article ? { sku: product.article } : {}), brand: { '@type': 'Brand', name: product.brand }, image: product.images, description: product.description, ...(product.price ? { offers: { '@type': 'Offer', price: product.price, priceCurrency: 'RUB' } } : {}) };
